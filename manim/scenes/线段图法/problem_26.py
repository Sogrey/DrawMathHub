"""
第26讲 还原问题（二）— 画图解题法（线段图法）

母题：吃了4颗后，分给剩下的一半多2颗，还剩5颗。原来多少？
答案：5+2=7，7×2=14，14+4=18

用法:
  cd manim/scenes/线段图法
  python -m manim problem_26.py Problem26Scene -ql
"""

from __future__ import annotations

import sys
from pathlib import Path

_SCENES = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(_SCENES / "_shared"))

from lesson_base import MathLessonScene  # noqa: E402
from manim import *
import numpy as np


class Problem26Scene(MathLessonScene):
    EXAMPLE_INDEX = 0

    LEFT_REMAIN = 5
    EXTRA = 2
    FIRST_TAKE = 4

    def construct(self):
        data = self.load_problem()
        self.init_video_recorder()
        mp = self.main_problem

        rem = self.LEFT_REMAIN
        extra = self.EXTRA
        take = self.FIRST_TAKE
        half = rem + abs(extra)
        after = half * 2
        original = after + take

        # ── 题型讲解（含片头）──
        with self.segment("intro", "题型讲解", "segments/01.mp4", "片头标题与题型讲解"):
            self.show_title(data["title"], subtitle=f"画图解题法 · {data['methodType']}")
            self.init_layout_after_title(prob_h=1.0)

            title_bottom = self._title_group.get_bottom()[1]
            zone_top = title_bottom - 0.45

            concept_title = self.safe_text("题型识别", font_size=34, color=YELLOW)
            concept_title.move_to(np.array([0, zone_top - 0.35, 0]))
            self.clamp_content(concept_title)

            s1_body = self.safe_text(
                "数量关系里出现「一半多几」或「一半少几」，",
                font_size=26, color=WHITE,
            )
            s1_body2 = self.safe_text(
                "要倒推出原来有多少——这就是还原问题（二）！",
                font_size=26, color=WHITE,
            )
            concept_body = VGroup(s1_body, s1_body2).arrange(
                DOWN, buff=0.22, aligned_edge=LEFT,
            )
            concept_body.next_to(concept_title, DOWN, buff=0.40)
            concept_body.move_to(np.array([0, concept_body.get_center()[1], 0]))
            self.clamp_content(concept_body)
            concept_block = VGroup(concept_title, concept_body)

            divider_y = concept_block.get_bottom()[1] - 0.40
            divider = Line(
                np.array([self.safe_left + 0.5, divider_y, 0]),
                np.array([self.safe_right - 0.5, divider_y, 0]),
                stroke_width=1.5, color=GREY_B, stroke_opacity=0.25,
            )

            feature_title = self.safe_text("线段图逆推的三个要点", font_size=32, color=YELLOW)
            feature_title.move_to(np.array([0, divider_y - 0.55, 0]))
            self.clamp_content(feature_title)

            features = [
                ("1", "先标出「还剩」与「多几」", "还剩+多几 = 剩下的一半"),
                ("2", "一半倒推变成两倍", "求出分给别人之前还剩多少"),
                ("3", "再加上一开始减掉的", "还原到原来的数量"),
            ]
            feature_groups = self.layout_numbered_features(
                features,
                top_y=feature_title.get_bottom()[1] - 0.45,
                row_step=1.08,
            )
            intro_all = VGroup(concept_block, divider, feature_title, *feature_groups)

            self.play(FadeIn(concept_block, shift=DOWN * 0.15), run_time=0.8)
            self.wait(2)
            self.play(FadeIn(divider), FadeIn(feature_title, shift=DOWN * 0.15), run_time=0.6)
            for row in feature_groups:
                self.play(FadeIn(row, shift=RIGHT * 0.25), run_time=0.5)
                self.wait(1)
            self.wait(2)
            self.play(FadeOut(intro_all), run_time=0.5)

        # ── 题目展示 ──
        prob_all = None
        with self.segment("question", "题目", "segments/02.mp4", "读题：一半多几"):
            prob_all = self.make_problem_box(
                f"题目：明明吃了{take}颗后，分给萌萌剩下的一半多{extra}颗，",
                f"最后还剩{rem}颗。明明原来有多少颗糖果？",
            )
            self.play(FadeIn(prob_all, shift=DOWN * 0.3), run_time=0.8)
            self.wait(4)

        draw_y = self.layout["draw_y"]
        diag = self.make_half_plus_restore_diagram(
            draw_y,
            left_remain=rem,
            extra=extra,
            first_take=take,
            show_hint=True,
            unit_label="颗",
        )

        # ── 图解1：画出整条线段 ──
        with self.segment("draw-1", "1", "segments/03.mp4", "画出线段图"):
            s1 = self.step_label("第一步：按数量关系画出线段")
            self.add(prob_all)
            self.play(FadeIn(s1), run_time=0.5)
            if len(diag["hint"]) > 0:
                self.play(FadeIn(diag["hint"]), run_time=0.3)
            self.play(Create(diag["main"]), run_time=0.5)
            self.play(FadeIn(diag["ticks"]), run_time=0.4)
            for bar in diag["bars"]:
                if len(bar) > 0:
                    self.play(FadeIn(bar), run_time=0.35)
            self.safe_subtitle(
                "从左到右：还剩、多几、一半、先吃掉的",
                wait=4,
            )
            self.play(FadeOut(s1), run_time=0.3)

        # ── 图解2：标还剩与多几 ──
        with self.segment("draw-2", "2", "segments/04.mp4", "标还剩与多几"):
            self.add(prob_all, diag["hint"], diag["line_core"])
            s2 = self.step_label("第二步：标出还剩和「一半多几」里的多几")
            self.play(FadeIn(s2), run_time=0.5)
            self.play(FadeIn(diag["left_block"]), run_time=0.5)
            self.play(FadeIn(diag["extra_block"]), run_time=0.5)
            self.safe_subtitle(
                f"还剩 {rem} 颗，分给别人时比一半多 {extra} 颗",
                wait=4,
            )
            self.play(FadeOut(s2), run_time=0.3)

        # ── 图解3：一半 = 还剩+多几 ──
        with self.segment("draw-3", "3", "segments/05.mp4", "求出一半"):
            self.add(
                prob_all, diag["hint"], diag["line_core"],
                diag["left_block"], diag["extra_block"],
            )
            s3 = self.step_label("第三步：还剩+多几，就是剩下糖果的一半")
            self.play(FadeIn(s3), run_time=0.5)
            self.play(FadeIn(diag["half_block"]), run_time=0.5)
            self.play(diag["half_eq_block"].animate.set_opacity(1), run_time=0.55)
            self.safe_subtitle(
                f"{rem}+{extra}={half}（颗），所以一半是 {half} 颗",
                wait=5,
            )
            self.play(FadeOut(s3), run_time=0.3)

        # ── 图解4：×2 再加先吃的 ──
        with self.segment("draw-4", "4", "segments/06.mp4", "还原原来数量"):
            self.add(
                prob_all, diag["hint"], diag["line_core"],
                diag["left_block"], diag["extra_block"],
                diag["half_block"], diag["half_eq_block"],
                diag["note_double"], diag["note_origin"],
            )
            s4 = self.step_label("第四步：一半×2，再加上先吃掉的")
            self.play(FadeIn(s4), run_time=0.5)
            if len(diag["hint"]) > 0:
                self.play(FadeOut(diag["hint"]), run_time=0.3)
            self.play(diag["note_double"].animate.set_opacity(1), run_time=0.5)
            self.play(FadeIn(diag["take_block"]), run_time=0.5)
            self.play(diag["note_origin"].animate.set_opacity(1), run_time=0.5)
            self.safe_subtitle(
                f"{half}×2={after}，再加 {take} → 原来 {original} 颗",
                wait=5,
            )
            self.wait(1)
            self.play(FadeOut(s4), run_time=0.3)

        diagram_all = VGroup(
            diag["line_core"],
            diag["left_block"], diag["extra_block"],
            diag["half_block"], diag["take_block"],
            diag["half_eq_block"], diag["note_double"], diag["note_origin"],
        )

        # ── 书面答题 ──
        with self.segment("written", "作答", "segments/07.mp4", "书面答题：列式作答"):
            self.add(prob_all, diagram_all)
            written_diagram_scale = self.place_diagram_for_written(diagram_all)

            left_x = self.written_left_x()
            rows_data = [
                (f"{rem}+{extra}={half}（颗）", "（先求一半）"),
                (f"{half}×2={after}（颗）", "（吃前还剩）"),
                (f"{after}+{take}={original}（颗）", "（原来有多少）"),
            ]
            rows = VGroup()
            for expr, note in rows_data:
                e = self.safe_text(expr, font_size=24, color=WHITE)
                n = self.safe_text(note, font_size=18, color=GREY_B)
                rows.add(VGroup(e, n).arrange(RIGHT, buff=0.16))
            formula_rows = rows.arrange(DOWN, buff=0.28, aligned_edge=LEFT)
            formula_rows.move_to(np.array([
                left_x + formula_rows.width / 2,
                self.written_left_y(0.55), 0,
            ]))
            formula_rows.align_to(np.array([left_x, 0, 0]), LEFT)
            self.clamp_content(formula_rows)

            for row in rows:
                self.play(FadeIn(row), run_time=0.45)
                self.wait(1.1)

            answer_text = self.safe_text(
                f"答：明明原来有{original}颗糖果。",
                font_size=26, color=YELLOW,
            )
            answer_text.next_to(formula_rows, DOWN, buff=0.42)
            answer_text.align_to(np.array([left_x, 0, 0]), LEFT)
            self.clamp_content(answer_text)
            highlight_box = SurroundingRectangle(
                answer_text, color=RED, buff=0.12, corner_radius=0.1,
            )
            self.play(FadeIn(answer_text, shift=UP * 0.15), run_time=0.7)
            self.play(Create(highlight_box), run_time=0.5)
            self.safe_subtitle("一半多几：先求一半，再×2 还原", wait=5)
            self.wait(2)
            self.play(
                FadeOut(formula_rows), FadeOut(answer_text),
                FadeOut(highlight_box), FadeOut(prob_all), run_time=0.5,
            )

        with self.segment("keypoints", "点拨", "segments/keypoints.mp4", "该题型的解题关键"):
            self.play_keypoints_only(
                mp["keyPoints"], wait=6,
                diagram=diagram_all, from_scale=written_diagram_scale,
            )

        with self.segment("end", "结尾", "segments/end.mp4", "片尾", gap_after=False):
            self.show_credits("THE END")

        self.finalize_lesson()
