"""
第46讲 行程综合问题（一）— 画图解题法（线段图法）

母题：甲 9 小时到；乙快 24 km/h，提前 2 小时到。求 A、B 距离。
答案：24×(9−2)=168；168÷2×9=756（千米）

用法:
  cd manim/scenes/线段图法
  python -m manim problem_46.py Problem46Scene -ql
"""

from __future__ import annotations

import sys
from pathlib import Path

_SCENES = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(_SCENES / "_shared"))

from lesson_base import MathLessonScene  # noqa: E402
from manim import *
import numpy as np


class Problem46Scene(MathLessonScene):
    EXAMPLE_INDEX = 0

    SLOW_HOURS = 9
    EARLY_HOURS = 2
    SPEED_DIFF = 24

    def construct(self):
        data = self.load_problem()
        self.init_video_recorder()
        mp = self.main_problem

        t_slow = self.SLOW_HOURS
        t_early = self.EARLY_HOURS
        dv = self.SPEED_DIFF
        t_fast = t_slow - t_early
        remain = dv * t_fast
        total = remain // t_early * t_slow

        # ── 题型讲解（含片头）──
        with self.segment("intro", "题型讲解", "segments/01.mp4", "片头标题与题型讲解"):
            self.show_title(data["title"], subtitle=f"画图解题法 · {data['methodType']}")
            self.init_layout_after_title(prob_h=1.0)

            title_bottom = self._title_group.get_bottom()[1]
            zone_top = title_bottom - 0.45

            concept_title = self.safe_text("题型识别", font_size=32, color=YELLOW)
            concept_title.move_to(np.array([0, zone_top - 0.32, 0]))
            self.clamp_content(concept_title)

            s1_body = self.safe_text(
                "路程、时间、速度三者互相牵制，",
                font_size=26, color=WHITE,
            )
            s1_body2 = self.safe_text(
                "题里藏着「快车提前到」这类条件——用线段图挖出来！",
                font_size=26, color=WHITE,
            )
            concept_body = VGroup(s1_body, s1_body2).arrange(
                DOWN, buff=0.20, aligned_edge=LEFT,
            )
            concept_body.next_to(concept_title, DOWN, buff=0.38)
            concept_body.move_to(np.array([0, concept_body.get_center()[1], 0]))
            self.clamp_content(concept_body)
            concept_block = VGroup(concept_title, concept_body)

            divider_y = concept_block.get_bottom()[1] - 0.38
            divider = Line(
                np.array([self.safe_left + 0.5, divider_y, 0]),
                np.array([self.safe_right - 0.5, divider_y, 0]),
                stroke_width=1.5, color=GREY_B, stroke_opacity=0.25,
            )

            feature_title = self.safe_text("提前到达型的三个要点", font_size=30, color=YELLOW)
            feature_title.move_to(np.array([0, divider_y - 0.52, 0]))
            self.clamp_content(feature_title)

            features = [
                ("1", "先求快车用时", "慢车时间减去提前时间"),
                ("2", "多走路程＝剩余路程", "速度差×快车用时＝慢车还剩的路"),
                ("3", "由剩余求慢速与全程", "剩余÷提前时间×慢车总时间"),
            ]
            feature_groups = self.layout_numbered_features(
                features,
                top_y=feature_title.get_bottom()[1] - 0.42,
                row_step=1.08,
            )
            intro_all = VGroup(concept_block, divider, feature_title, *feature_groups)

            self.play(FadeIn(concept_block, shift=DOWN * 0.15), run_time=0.8)
            self.wait(2)
            self.play(FadeIn(divider), FadeIn(feature_title, shift=DOWN * 0.15), run_time=0.6)
            for row in feature_groups:
                self.play(FadeIn(row, shift=RIGHT * 0.25), run_time=0.5)
                self.wait(1)
            self.wait(2)
            self.play(FadeOut(intro_all), run_time=0.5)

        # ── 题目展示 ──
        prob_all = None
        with self.segment("question", "题目", "segments/02.mp4", "读题：甲乙同向同程"):
            prob_all = self.make_problem_box(
                f"题目：甲、乙同时从A开往B。甲{t_slow}小时到；乙每小时快{dv}千米，",
                f"比甲提前{t_early}小时到。求A、B两地距离。",
            )
            self.play(FadeIn(prob_all, shift=DOWN * 0.3), run_time=0.8)
            self.wait(4)

        draw_y = self.layout["draw_y"]
        diag = self.make_travel_early_diagram(
            draw_y,
            slow_hours=t_slow,
            early_hours=t_early,
            speed_diff=dv,
            show_hint=True,
        )

        # ── 图解1：画出甲乙全程线段 ──
        with self.segment("draw-1", "1", "segments/03.mp4", "画出甲乙线段图"):
            s1 = self.step_label("第一步：画出甲、乙从A到B的线段图")
            self.add(prob_all)
            self.play(FadeIn(s1), run_time=0.5)
            if len(diag["hint"]) > 0:
                self.play(FadeIn(diag["hint"]), run_time=0.3)
            self.play(
                FadeIn(diag["dash_a"]), FadeIn(diag["dash_b"]),
                FadeIn(diag["lab_a"]), FadeIn(diag["lab_b"]),
                run_time=0.45,
            )
            self.play(
                Create(diag["slow_main"]), Create(diag["slow_remain"]),
                FadeIn(diag["slow_ticks"]), FadeIn(diag["slow_name_lab"]),
                FadeIn(diag["t7"]),
                run_time=0.7,
            )
            self.play(
                Create(diag["fast_line"]),
                FadeIn(diag["fast_ticks"]), FadeIn(diag["fast_name_lab"]),
                run_time=0.55,
            )
            self.safe_subtitle(
                "两车同从A到B，线段一样长，表示路程相同",
                wait=4,
            )
            self.play(FadeOut(s1), run_time=0.3)

        # ── 图解2：标乙用时与甲剩余时间 ──
        with self.segment("draw-2", "2", "segments/04.mp4", "标出时间分段"):
            self.add(
                prob_all, diag["hint"], diag["dash_a"], diag["dash_b"],
                diag["place_labs"], diag["slow_row"], diag["fast_row"],
            )
            s2 = self.step_label("第二步：标出乙的用时和甲还剩的时间")
            self.play(FadeIn(s2), run_time=0.5)
            self.play(diag["note_time"].animate.set_opacity(1), run_time=0.5)
            self.play(FadeIn(diag["dash_split"]), run_time=0.4)
            self.play(
                diag["t7_fast"].animate.set_opacity(1),
                diag["t2"].animate.set_opacity(1),
                run_time=0.55,
            )
            self.safe_subtitle(
                f"乙用时 {t_slow}−{t_early}={t_fast} 小时；甲此时还剩 {t_early} 小时",
                wait=5,
            )
            self.play(FadeOut(s2), run_time=0.3)

        # ── 图解3：剩余路程 = 速度差×乙用时 ──
        with self.segment("draw-3", "3", "segments/05.mp4", "求出剩余路程"):
            self.add(
                prob_all, diag["hint"], diag["guides"], diag["place_labs"],
                diag["slow_row"], diag["fast_row"], diag["notes"],
            )
            s3 = self.step_label("第三步：乙多走的路程，就是甲还剩的路程")
            self.play(FadeIn(s3), run_time=0.5)
            self.play(
                diag["remain_brace"].animate.set_opacity(1),
                diag["remain_q"].animate.set_opacity(1),
                run_time=0.5,
            )
            self.play(diag["note_gap"].animate.set_opacity(1), run_time=0.5)
            self.wait(0.6)
            self.play(
                diag["remain_q"].animate.set_opacity(0),
                diag["remain_ans"].animate.set_opacity(1),
                run_time=0.55,
            )
            self.safe_subtitle(
                f"乙每小时快 {dv} 千米，{t_fast} 小时多走 {dv}×{t_fast}={remain} 千米",
                wait=5,
            )
            self.play(FadeOut(s3), run_time=0.3)

        # ── 图解4：求全程 ──
        with self.segment("draw-4", "4", "segments/06.mp4", "求出全程距离"):
            self.add(
                prob_all, diag["hint"], diag["guides"], diag["place_labs"],
                diag["slow_row"], diag["fast_row"], diag["remain_block"],
                diag["notes"],
            )
            s4 = self.step_label("第四步：由剩余路程求出甲速，再求全程")
            self.play(FadeIn(s4), run_time=0.5)
            if len(diag["hint"]) > 0:
                self.play(FadeOut(diag["hint"]), run_time=0.25)
            self.play(diag["note_total_q"].animate.set_opacity(1), run_time=0.45)
            self.wait(0.6)
            self.play(
                diag["note_total_q"].animate.set_opacity(0),
                diag["note_total_ans"].animate.set_opacity(1),
                run_time=0.55,
            )
            self.safe_subtitle(
                f"{remain}÷{t_early}×{t_slow}={total}（千米）",
                wait=5,
            )
            self.wait(1)
            self.play(FadeOut(s4), run_time=0.3)

        diagram_all = VGroup(
            diag["guides"], diag["place_labs"],
            diag["slow_row"], diag["fast_row"],
            diag["remain_block"], diag["notes"],
        )

        # ── 列式作答 ──
        with self.segment("written", "作答", "segments/07.mp4", "书面答题：列式作答"):
            self.add(prob_all, diagram_all)
            written_diagram_scale = self.place_diagram_for_written(diagram_all)

            left_x = self.written_left_x()
            f1 = self.safe_text(
                f"{dv}×({t_slow}−{t_early})={remain}（千米）",
                font_size=24, color=WHITE,
            )
            f1.move_to(np.array([
                left_x + f1.width / 2,
                self.written_left_y(0.55), 0,
            ]))
            f1.align_to(np.array([left_x, 0, 0]), LEFT)
            self.clamp_content(f1)

            f2 = self.safe_text(
                f"{remain}÷{t_early}×{t_slow}={total}（千米）",
                font_size=24, color=WHITE,
            )
            f2.next_to(f1, DOWN, buff=0.28)
            f2.align_to(np.array([left_x, 0, 0]), LEFT)
            self.clamp_content(f2)

            answer_text = self.safe_text(
                f"答：A、B两地间的距离是{total}千米。",
                font_size=22, color=YELLOW,
            )
            answer_text.next_to(f2, DOWN, buff=0.36)
            answer_text.align_to(np.array([left_x, 0, 0]), LEFT)
            self.clamp_content(answer_text)
            highlight_box = SurroundingRectangle(
                answer_text, color=RED, buff=0.12, corner_radius=0.1,
            )

            self.play(FadeIn(f1), run_time=0.5)
            self.wait(0.8)
            self.play(FadeIn(f2), run_time=0.5)
            self.wait(0.8)
            self.play(FadeIn(answer_text, shift=UP * 0.15), run_time=0.7)
            self.play(Create(highlight_box), run_time=0.5)
            self.safe_subtitle("剩余路程÷提前时间×慢车总时间", wait=5)
            self.wait(2)
            self.play(
                FadeOut(f1), FadeOut(f2), FadeOut(answer_text),
                FadeOut(highlight_box), FadeOut(prob_all), run_time=0.5,
            )

        with self.segment("keypoints", "点拨", "segments/keypoints.mp4", "该题型的解题关键"):
            self.play_keypoints_only(
                mp["keyPoints"], wait=6,
                diagram=diagram_all, from_scale=written_diagram_scale,
            )

        with self.segment("end", "结尾", "segments/end.mp4", "片尾", gap_after=False):
            self.show_credits("THE END")

        self.finalize_lesson()
