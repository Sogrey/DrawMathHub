...
母题：一根木头被锯3次后，每段长4米，这根木头原来长多少米？
答案：3+1=4（段），4×4=16（米）

用法:
  cd manim/scenes/图示法
  python -m manim problem_8.py Problem8Scene -qh

渲染后:
  python ..\_shared\post_render.py --lesson 8 \\
    --rendered media\videos\problem_8\1080p60\Problem8Scene.mp4
"""
...
class Problem8Scene(MathLessonScene):
    EXAMPLE_INDEX = 0
    ...
    def construct(self):
        ...
        with self.segment("intro", "题型讲解", "segments/01.mp4", "片头标题与题型讲解"):
            ...
            concept_title = self.safe_text("什么是间隔问题？", font_size=36, color=YELLOW)
            concept_title.move_to(np.array([0, zone_top - 0.35, 0]))
            self.clamp_content(concept_title)

            s1_body = self.safe_text(
                "锯木头时，每锯一次就多一段，", font_size=28, color=WHITE,
            )
            s1_body2 = self.safe_text(
                "段数和锯的次数差 1——这就是间隔问题！", font_size=28, color=WHITE,
            )
            ...
            feature_title = self.safe_text("锯木头间隔问题的三个特征", font_size=34, color=YELLOW)
            feature_title.move_to(np.array([0, divider_y - 0.60, 0]))
            self.clamp_content(feature_title)

            features = [
                ("1", "用实线表示木头", "用虚线标出每次锯的位置"),
                ("2", "段数 = 锯的次数 + 1", "比如：锯3次 → 4段"),
                ("3", "原长 = 段数 × 每段长度", "先求段数，再求总长度"),
            ]
        ...
        # ── 题目展示 ──
        prob_all = None
        with self.segment("question", "题目", "segments/02.mp4", "读题：锯3次每段4米"):
        ...
        diag = self.make_saw_diagram(self.CUTS, self.SEG_LEN, draw_y)
        segments = diag["segments"]
        total_len = diag["total_len"]

        # ── 图解1：画木头 ──
        with self.segment("draw-1", "1", "segments/03.mp4", "画一根线表示木头"):
            s1 = self.step_label("第一步：用一根线表示这根木头")
            self.add(prob_all)
            self.play(FadeIn(s1), run_time=0.5)
            self.play(FadeIn(diag["hint"]), run_time=0.4)
            self.play(Create(diag["wood"]), run_time=0.85)
            self.safe_subtitle("先画一根线，代表整根木头", wait=4)
            self.play(FadeOut(s1), run_time=0.3)

        # ── 图解2：锯3次 ──
        with self.segment("draw-2", "2", "segments/04.mp4", "用虚线标3次锯的位置"):
            self.add(prob_all, diag["hint"], diag["wood"])
            s2 = self.step_label("第二步：用虚线标出锯了3次的位置")
            self.play(FadeIn(s2), run_time=0.5)
            for mark in diag["cut_marks"]:
                self.play(
                    Create(mark["cut_line"]),
                    FadeIn(mark["cut_label"], shift=DOWN * 0.08),
                    run_time=0.45,
                )
            self.safe_subtitle("锯3次，就有3条竖虚线", wait=4)
            self.play(FadeOut(s2), run_time=0.3)

        # ── 图解3：标4段 + 每段4米 ──
        with self.segment("draw-3", "3", "segments/05.mp4", "标出4段每段4米"):
            self.add(prob_all, diag["hint"], diag["wood"], diag["cuts_block"])
            s3 = self.step_label("第三步：标出4段，每段长4米")
            self.play(FadeIn(s3), run_time=0.5)
            for mark in diag["seg_marks"]:
                self.play(FadeIn(mark["parts"], shift=UP * 0.06), run_time=0.35)
            self.play(GrowFromCenter(diag["seg_len_brace"]), run_time=0.45)
            self.play(
                FadeIn(diag["seg_len_text"]),
                FadeIn(diag["seg_len_note"]),
                run_time=0.45,
            )
            self.safe_subtitle("锯3次锯成4段，每段都是4米", wait=4)
            self.play(FadeOut(s3), run_time=0.3)

        # ── 图解4：段数=次数+1 ──
        with self.segment("draw-4", "4", "segments/06.mp4", "段数等于锯的次数加1"):
            self.add(
                prob_all, diag["hint"], diag["wood"],
                diag["cuts_block"], diag["segs_block"], diag["len_block"],
            )
            s4 = self.step_label("第四步：段数 = 锯的次数 + 1")
            self.play(FadeIn(s4), run_time=0.5)
            self.play(FadeIn(diag["relation_note"], shift=DOWN * 0.08), run_time=0.45)
            self.play(FadeIn(diag["rule_label"], shift=DOWN * 0.08), run_time=0.55)
            self.safe_subtitle("段数比锯的次数多1，3+1=4段", wait=4)
        ...
        with self.segment("written", "作答", "segments/07.mp4", "书面答题：列式作答"):
            ...
            step1 = self.safe_text(
                f"{self.CUTS}+1={segments}（段）", font_size=28, color=WHITE,
            )
            explain = self.safe_text(
                f"锯断后总共{segments}段，每段{self.SEG_LEN}米，那么原木长度为：",
                font_size=24, color=GREY_B,
            )
            step2 = self.safe_text(
                f"{segments}×{self.SEG_LEN}={total_len}（米）",
            ...
            self.wait(1.0)
            self.play(FadeIn(explain), run_time=0.55)
            self.play(FadeIn(step2), run_time=0.5)
            self.wait(2)

            answer_text = self.safe_text(
                f"答：这根木头原来长{total_len}米。",
            ...
            self.safe_subtitle("先求段数，再求总长度", wait=5)
            self.wait(2)
            self.play(
